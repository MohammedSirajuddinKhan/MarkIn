...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function updateTemplateCountLabel() {
    ...
    templateCountLabel.textContent = `Template totals: Students ${templateState.students}, Teachers ${templateState.teachers}`;
  }
  ...
  function updateImportChecklistFromData(data) {
    ...
    console.log(
      `📊 Updating checklist from data: ${studentsCount} students, ${teachersCount} teachers`,
    );
      ...
      form.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          showToast({
            ...
            message: `Uploaded ${payload.uploaded || 0}. Template now has ${payload.total} ${type} entries ready for review.`,
            type: "success",
          });
        } catch (error) {
          showToast({
  ...
  const refreshMappingsButton = document.querySelector(
    "[data-refresh-mappings]",
  );
  ...
  const closeChangePasswordButton = document.querySelector(
    "[data-close-change-password]",
  );
  const cancelChangePasswordButton = document.querySelector(
    "[data-cancel-change-password]",
  );
  ...
  function setupLiveUpdates() {
    ...
    liveEventSource.addEventListener("data_import", (event) => {
      ...
      showToast({
        title: "Data Imported",
        message: `${data.count || 0} ${data.dataType || "records"} imported successfully`,
        type: "success",
      });
      loadStats();
      loadActivity();
  ...
  const addTeacherDivisionsInput = document.querySelector(
    "#addTeacherDivisions",
  );
  ...
  function collectMappingsFromContainer(container) {
    return Array.from(
      ...
      .map((row) => {
        const subject = row
          ?.querySelector("[data-mapping-subject]")
          ?.value?.trim();
        const year = row?.querySelector("[data-mapping-year]")?.value?.trim();
        const semester = row
          ?.querySelector("[data-mapping-semester]")
          ?.value?.trim();
        const stream = row
          ?.querySelector("[data-mapping-stream]")
          ?.value?.trim();
  ...
  function createTeacherMappingRow(prefill = {}) {
    ...
    if (prefill.semester && semesterSelect)
      semesterSelect.value = prefill.semester;
  ...
  function validateTeacherEditFormInputs({ silent = false } = {}) {
    const teacherName =
      document.querySelector("#editTeacherName")?.value?.trim() || "";
    ...
    if (teacherName && !hasOnlyLettersAndSpaces(teacherName)) {
      const message =
        "Warning: Teacher name cannot contain numbers or special characters.";
    ...
    if (commonDivision && !hasOnlyAlphaNumSpacesCommas(commonDivision)) {
      const message =
        "Warning: Common divisions can only contain letters, numbers, commas, and spaces.";
      setEditWarningLabel(editTeacherWarningLabel, message);
      if (!silent) {
    ...
    if (invalidMapping) {
      const message =
        "Warning: Subject mapping fields cannot contain special characters.";
      setEditWarningLabel(editTeacherWarningLabel, message);
      if (!silent) {
        showToast({
          title: "Invalid mapping fields",
          message,
          type: "warning",
        });
      }
  ...
  function setSelectedTeacherInList(teacherId) {
    const buttons =
      editTeacherList?.querySelectorAll("[data-edit-teacher-item]") || [];
  ...
  async function loadTeacherForEdit(teacherId) {
    try {
      ...
      if (editTeacherIdInput)
        editTeacherIdInput.value = teacher.teacherId || "";
      if (editTeacherNameInput)
        editTeacherNameInput.value = teacher.teacherName || "";
  ...
  openAddTeacherTabButton?.addEventListener("click", async () => {
    if (!teacherFormStreams.length) {
      ...
      streamSelects?.forEach((select) =>
        populateStreamOptionsForMapping(select),
      );
    }
    showAddTeacherTab();
  });
  ...
  editTeacherForm?.addEventListener("submit", async (event) => {
    ...
    const isEditTeacherFormValid = [
      ...
      ...Array.from(
        editTeacherMappingsContainer?.querySelectorAll(
          "[data-mapping-subject]",
        ) || [],
      ).map((input) => validateInputNow(input, validateSubjectValue)),
    ].every(Boolean);
    ...
    try {
      ...
      showToast({
        ...
        message:
          response.message || "Teacher information updated successfully.",
        type: "success",
      });
  ...
  const refreshStudentsButton = document.querySelector(
    "[data-refresh-students]",
  );
  ...
  const submitAddStudentButton = document.querySelector(
    "[data-submit-add-student]",
  );
  ...
  const editStudentDivisionInput = document.querySelector(
    "#editStudentDivision",
  );
  ...
  const bulkStudentDivisionSelect = document.querySelector(
    "#bulkStudentDivision",
  );
  ...
  function applyTableSearch({
    ...
    const existingNoMatchRow = tbody.querySelector(
      "[data-inline-search-empty]",
    );
  ...
  function validateStudentEditFormInputs({ silent = false } = {}) {
    const studentName =
      document.querySelector("#editStudentName")?.value?.trim() || "";
    const year =
      document.querySelector("#editStudentYear")?.value?.trim() || "";
    const stream =
      document.querySelector("#editStudentStream")?.value?.trim() || "";
    const division =
      document.querySelector("#editStudentDivision")?.value?.trim() || "";

    if (studentName && !hasOnlyLettersAndSpaces(studentName)) {
      const message =
        "Warning: Student name cannot contain numbers or special characters.";
    ...
    if (
      (year && !hasOnlyAlphaNumSpaces(year)) ||
      (stream && !hasOnlyAlphaNumSpaces(stream)) ||
      (division && !hasOnlyAlphaNumSpaces(division))
    ) {
      const message =
        "Warning: Year, Stream, and Division cannot contain special characters.";
      setEditWarningLabel(editStudentWarningLabel, message);
      if (!silent) {
        showToast({
          title: "Invalid student fields",
          message,
          type: "warning",
        });
      }
  ...
  async function loadStudentsForEditList() {
    ...
    try {
      ...
      editStudentList.innerHTML =
        '<p class="tagline">Unable to load students.</p>';
      showToast({
  ...
  async function loadStudentForEdit(studentId) {
    ...
    try {
      ...
      const editStudentNameInput = document.querySelector("#editStudentName");
      const editStudentRollNoInput =
        document.querySelector("#editStudentRollNo");
      const editStudentYearSelect = document.querySelector("#editStudentYear");
      const editStudentStreamInput =
        document.querySelector("#editStudentStream");
      const editStudentDivisionInput = document.querySelector(
        "#editStudentDivision",
      );
      ...
      if (editStudentNameInput)
        editStudentNameInput.value = student.studentName || "";
      if (editStudentRollNoInput) {
        editStudentRollNoInput.value = student.rollNo ?? "";
      }
      if (editStudentYearSelect)
        editStudentYearSelect.value = student.year || "";
      if (editStudentStreamInput)
        editStudentStreamInput.value = student.stream || "";
  ...
  async function loadStudentsInfo({ showExtras = true } = {}) {
    ...
    try {
      ...
      const data = await apiFetch(
        `/api/admin/students-info?${params.toString()}`,
      );
      ...
      if (showExtras) {
        ...
        `,
          ...
          subjectsList.innerHTML =
            '<p class="tagline">No subjects available</p>';
        }
        ...
        `,
          ...
          teachersList.innerHTML =
            '<p class="tagline">No teachers available</p>';
        }
      ...
      <tr>
        ...
        <td>
          <button
            ...
            style="padding: 0.3rem 0.75rem; font-size: 0.82rem; min-width: 95px; ${
  ...
  addStudentForm?.addEventListener("submit", async (event) => {
    ...
    const studentName = document
      ...
      ?.value?.trim();
    const rollNoValue = document
      .querySelector("#addStudentRollNo")
      ?.value?.trim();
    ...
    const division = document
      .querySelector("#addStudentDivision")
      ?.value?.trim();
  ...
  editStudentForm?.addEventListener("submit", async (event) => {
    ...
    const studentName = document
      ...
      ?.value?.trim();
    const rollNoValue = document
      .querySelector("#editStudentRollNo")
      ?.value?.trim();
    ...
    const division = document
      .querySelector("#editStudentDivision")
      ?.value?.trim();
  ...
  async function showCurrentSessionsList() {
    ...
    try {
      ...
      if (!currentSessions || currentSessions.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6">No ongoing sessions found</td></tr>';
        return;
      }
  ...
  try {
    ...
    if (isDataFeedPaused) {
      console.log(
        "⏸️ Live data feed is paused; loading current database state without SSE updates.",
      );
    }
    ...
    const searchResultsModal = document.querySelector(
      "[data-search-results-modal]",
    );
    const studentDetailsModal = document.querySelector(
      "[data-student-details-modal]",
    );
    const teacherDetailsModal = document.querySelector(
      "[data-teacher-details-modal]",
    );
    const sessionAttendanceModal = document.querySelector(
      "[data-session-attendance-modal]",
    );
    const searchResultsContent = document.querySelector(
      "[data-search-results-content]",
    );
    const studentDetailsContent = document.querySelector(
      "[data-student-details-content]",
    );
    const teacherDetailsContent = document.querySelector(
      "[data-teacher-details-content]",
    );
    const sessionAttendanceContent = document.querySelector(
      "[data-session-attendance-content]",
    );
    const closeSearchResultsBtn = document.querySelector(
      "[data-close-search-results]",
    );
    const closeStudentDetailsBtn = document.querySelector(
      "[data-close-student-details]",
    );
    const closeTeacherDetailsBtn = document.querySelector(
      "[data-close-teacher-details]",
    );
    const closeSessionAttendanceBtn = document.querySelector(
      "[data-close-session-attendance]",
    );
    const sessionSearchInput = document.querySelector(
    ...
    async function performSearch() {
      ...
      if (!searchQuery) {
        showToast({
          ...
          message:
            "Please enter search term (ID, Name, Roll No, Subject, etc.)",
          type: "warning",
        });
        return;
      }
      ...
      try {
        ...
        const studentResponse = await apiFetch(
          `/api/admin/search/student/${encodeURIComponent(searchQuery)}`,
        );
      ...
      try {
        ...
        const teacherResponse = await apiFetch(
          `/api/admin/search/teacher/${encodeURIComponent(searchQuery)}`,
        );
    ...
    function displayStudentDetails(student) {
      ...
      const attendanceColor =
        attendancePercentage >= 75 ? "#27ae60" : "#e74c3c";
      ...
      const attendanceSection = studentDetailsContent.querySelector(
        "[data-attendance-clickable]",
      );
    ...
    async function showSessionAttendance(studentId) {
      try {
        ...
        const response = await apiFetch(
          `/api/admin/student/${encodeURIComponent(studentId)}/sessions`,
        );
        ...
        <table style="width: 100%; border-collapse: collapse; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tbody>
            ${sessions
              .map((session, index) => {
                const isHighlighted =
                  highlightQuery && shouldHighlightRow(session, highlightQuery);
                const rowStyle = `
    ...
    function shouldHighlightRow(session, query) {
      ...
      const statusText =
        session.status?.toUpperCase() === "P" ? "present" : "absent";
      return (
